...
use crate::utils::task::{task_yield, task_yield_interruptable};
use axerrno::{LinuxError, LinuxResult};
use axfs_ng::api::FileFlags;
use axio::PollState;
use axnet::{RecvFlags, TcpSocket, UdpSocket};
use axsync::Mutex;
use linux_raw_sys::general::{O_CLOEXEC, O_NONBLOCK};
use linux_raw_sys::net::{MSG_PEEK, MSG_TRUNC, SOCK_DGRAM, SOCK_SEQPACKET};
...
impl Socket {
    ...
    fn send(&self, mut buf: &[u8]) -> LinuxResult<usize> {
        match self {
            Socket::Udp(udpsocket) => Ok(udpsocket.lock().send(buf)?),
            Socket::Tcp(tcpsocket) => Ok(tcpsocket.lock().send(&mut buf)?),
        }
    }

    fn recv(&self, mut buf: &mut [u8], flags: RecvFlags) -> LinuxResult<usize> {
        match self {
            Socket::Udp(udpsocket) => Ok(udpsocket.lock().recv_from(buf).map(|e| e.0)?),
            Socket::Tcp(tcpsocket) => Ok(tcpsocket.lock().recv(&mut buf, flags)?),
        }
    }
    ...
    fn recvfrom(
        ...
        mut buf: &mut [u8],
        flags: RecvFlags,
    ) -> LinuxResult<(usize, Option<SocketAddr>)> {
        match self {
...
impl FileLike for Socket {
    fn read(&self, buf: &mut [u8]) -> LinuxResult<usize> {
        debug!("reading {} bytes from socket", buf.len());
        self.recv(buf, RecvFlags::empty())
    }

    fn write(&self, buf: &[u8]) -> LinuxResult<usize> {
        debug!("writing {} bytes to socket", buf.len());
        self.send(buf)
    }
...
pub fn sys_socketpair(
    ...
    *fds.get_as_mut_ref()? = [
        fd_add(Arc::new(sock1), fd_flags.clone())?,
        fd_add(Arc::new(sock2), fd_flags)?,
    ];
    Ok(0)
}
...
pub fn sys_recvfrom(
    ...
    let flags = RecvFlags::from_bits_truncate(flag as _);
